...
public sealed abstract class StatType<Value> implements Comparator<Value> permits DoubleStat,
    IntegerStat, StringStat,
    PercentStat {
  ...
  public static StatType<?> wrapStatType(String type,
    ...
    return switch (type.toLowerCase()) {
      case IntegerStat.NAME ->
          new IntegerStat(name, displayName, defaultValue, displayIndex, displayLineIndex,
              globalDisplay, globalDisplayIndex, globalDisplayLineIndex);
      case DoubleStat.NAME ->
          new DoubleStat(name, displayName, defaultValue, displayIndex, displayLineIndex,
              globalDisplay, globalDisplayIndex, globalDisplayLineIndex);
      case PercentStat.NAME ->
          new PercentStat(name, displayName, defaultValue, displayIndex, displayLineIndex,
              globalDisplay, globalDisplayIndex, globalDisplayLineIndex);
      case StringStat.NAME ->
          new StringStat(name, displayName, defaultValue, displayIndex, displayLineIndex,
              globalDisplay, globalDisplayIndex, globalDisplayLineIndex);
      default -> null;
    };
  }
